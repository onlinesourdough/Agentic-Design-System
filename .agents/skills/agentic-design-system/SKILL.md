...
5. Use `$review-design` as the review method. Record its result and concrete
   ...
   decision by itself.
6. Write structured input, output, and proof files under
   ...
   predecessor.
7. Append exactly one small JSON object to
...
8. Promote into `examples/<slug>/` only when the owner explicitly requests
...
9. Update `workspace/state/active.json` only as a small current-state pointer;
